...
layout(location = 4) in vec3 fragTangent;      // Unused in default shader (for compatibility)
layout(location = 5) in vec3 fragBitangent;    // Unused in default shader (for compatibility)
layout(location = 6) flat in int fragMaterialIndex;  // Unused in default shader (for compatibility)
...
void main() {
    ...
    // White texture (1,1,1) → vertex color shows through
    // Colored texture → texture modulates the vertex color
    vec3 baseColor = fragColor * textureColor;
